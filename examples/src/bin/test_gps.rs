...
async fn main(_s: Spawner) {
    ...
    // GPS enable is active low on the gen2 board
    let mut gps_en = Output::new(p.P0_00, Level::Low, OutputDrive::Standard);
    gps_en.set_low();
    ...
    // send_at!("AT+CEREG=2");
    // Timer::after_secs(1).await;
    // match nrf_modem::send_at::<64>("AT+CEREG?").await {
    //     Ok(rsp) => {
    ...
    //     Err(err) => {
    //         #[cfg(feature="debug")]
    //         error!("Error running `AT+CEREG`: {} ", err);
    //         #[cfg(not(feature="debug"))]
    //         error!("Error running `AT+CEREG`");
    //     }
    // }
    // match nrf_modem::send_at::<64>("AT+COPS?").await {
    //     Ok(rsp) => {
    ...
    //     Err(err) => {
    //         #[cfg(feature="debug")]
    //         error!("Error running `AT+COPS`: {} ", err);
    //         #[cfg(not(feature="debug"))]
    //         error!("Error running `AT+COPS`");
    //     }
    // }
    ...
    let config = nrf_modem::GnssConfig {
        ...
        use_case: nrf_modem::GnssUsecase {
            low_accuracy: true,
            scheduled_downloads_disable: true,
        },
    ...
    static SYSTEM_CLOCK_AND_TOWS: &[u8] = include_bytes!("../../assistance_data/system_clock_and_tows.bin");
    ...
    unsafe {
        ...
        unsafe {
            let res = nrfxlib_sys::nrf_modem_gnss_agnss_write(
                p, len, nrfxlib_sys::NRF_MODEM_GNSS_AGNSS_GPS_UTC_PARAMETERS as u16);
            if res != 0 {
                error!("UTC_PARAMS: {}", res);
            }
        }
        ...
        unsafe {
            ...
            if res != 0 {
                error!("SYSTEM_CLOCK_AND_TOWS: {}", res);
            }
        }
